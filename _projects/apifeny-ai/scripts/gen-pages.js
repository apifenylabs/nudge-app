...
// Read clean reference
const refLines = fs.readFileSync('/tmp/clean-ref.txt', 'utf8').split('\n');

// For the PERSONAL_ASSISTANT reference, we need to understand its structure.
// We'll build new pages using the same structural pattern but with different content.
// 
// Strategy: read the full reference, identify all "slots" that need replacement,
// and substitute per-slug content.

// First, let's dump the reference structure to understand it
const ref = refLines.join('\n');
...
const DATA = {
  'ai-for-hr-and-recruiting': {
    ...
    plainTitle: 'AI for HR & Recruiting',
    ...
    fomoPrice: '$15',
    fomoCopies: '100+',
    finalHeading: 'Ready to Transform Your HR Workflow?',
    finalDesc: 'Get the complete 40+ page PDF playbook. Every strategy, prompt, and system you need to streamline hiring and HR operations.',
    guaranteeDesc: 'If the AI for HR &amp; Recruiting playbook doesn&#39;t help you reduce time-to-hire by at least 30% within 30 days, I&#39;ll refund every cent. No questions asked.',
  },
  'ai-for-social-media-management': {
    ...
    plainTitle: 'AI for Social Media Management',
    ...
    fomoPrice: '$12',
    fomoCopies: '200+',
    finalHeading: 'Ready to Supercharge Your Social Media?',
    finalDesc: 'Get the complete 35+ page PDF playbook. Every strategy, prompt, and system you need to scale your social media presence.',
    guaranteeDesc: 'If the AI for Social Media Management playbook doesn&#39;t help you increase engagement by at least 30% within 30 days, I&#39;ll refund every cent. No questions asked.',
  },
  'ai-for-personal-finance': {
    fnInner: 'AiForPersonalFinanceInner',
    fnPage: 'AiForPersonalFinancePage',
    title: 'AI for Personal Finance &amp; Budgeting',
    plainTitle: 'AI for Personal Finance & Budgeting',
    ...
    fomoPrice: '$15',
    fomoCopies: '150+',
    finalHeading: 'Ready to Master Your Money?',
    finalDesc: 'Get the complete 35+ page PDF playbook. Every strategy, prompt, and system you need to take control of your finances.',
    guaranteeDesc: 'If the AI for Personal Finance &amp; Budgeting playbook doesn&#39;t help you save at least $200/month within 30 days, I&#39;ll refund every cent. No questions asked.',
  },
};
...
function buildSections(sections) {
  ...
  return JSON.stringify(items, null, 2)
    .replace(/"iconName":/g, '"icon":')
    .replace(/"colorName":/g, '"color":');
}
...
function buildSectionDetails(data) {
  return data.sections.map(s => `
          <details
            key="${s.id}"
            className="group rounded-xl border border-tech-500/20 bg-tech-700/50 overflow-hidden transition hover:border-violet-500/20"
          >
...
${s.items.map(item => `                <li key="${s.items.indexOf(item)}" className="flex items-start gap-2 text-xs sm:text-sm text-tech-100">
...
function buildIncludeGrid(items) {
