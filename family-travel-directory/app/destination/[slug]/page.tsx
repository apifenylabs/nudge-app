    ...
    async function load() {
      try {
        ...
        if (res.ok) {
          ...
          const found = data.find((d) => d.id === slug) || null;
          setDestination(found);

          // Set SEO metadata
          if (found) {
            document.title = `${found.name} | Family Travel Asia - ${found.city}, ${found.country}`;
            ...
            metaDesc.setAttribute(
              'content',
              `${found.name} in ${found.city}, ${found.country} — ${found.description}. Read real parent reviews, tips, and itineraries.`
            );

            // Open Graph tags
            const ogTags = [
              { property: 'og:title', content: `${found.name} | Family Travel Asia` },
              { property: 'og:description', content: found.description },
              { property: 'og:image', content: found.imageUrl },
              { property: 'og:url', content: `https://familytravelasia.com/destination/${found.id}` },
              { property: 'og:type', content: 'website' },
              { property: 'og:site_name', content: 'Family Travel Asia' },
            ];
            ...
            const twitterTags = [
              ...
              { name: 'twitter:title', content: `${found.name} | Family Travel Asia` },
              { name: 'twitter:description', content: found.description },
              { name: 'twitter:image', content: found.imageUrl },
            ];
            ...
            document.title = 'Destination Not Found | Family Travel Asia';
          }
        }
        setLoading(false);
      } catch (e) {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-gray-500 text-sm mb-6">
            We couldn&apos;t find a destination matching this page. It may have been moved or doesn&apos;t exist yet.
          </p>
          <Link
            href="/"
