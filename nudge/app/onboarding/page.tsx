...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (membership) {
        ...
        // Family already exists — skip to task creation
        setStep(2)
      }
      setLoading(false)
    }
  ...
  useEffect(() => {
    // Generate invite link when we have familyId and reach step 3
    if (familyId && step === 3) {
        ...
        {step === 5 ? (
          ...
          <>
            ...
            <OnboardingStepIndicator
              steps={steps.filter((s) => s.number <= 4)}
              currentStep={step}
            />
            ...
            >
              ...
              {step === 4 && (
                <StepConnectTelegram onComplete={() => setStep(5)} />
              )}
            </div>
          </>
        )}
