...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const { data: authData, error: authError } = await supabaseFn().auth.signUp({
        email,
        password,
        options: {
          data: {
      ...
      if (authData.user) {
        const { error: profileError } = await supabaseFn()
          .from('users')
          .insert({
            id: authData.user.id,
            email,
        ...
        const { data: familyData, error: familyError } = await supabaseFn()
          .from('families')
          .insert({
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground">Join Nudge</h1>
          <p className="text-sm text-muted-foreground mt-1">A calmer household starts here</p>
        </div>
