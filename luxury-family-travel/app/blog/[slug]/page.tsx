...
function markdownToHtml(markdown: string): string {
  // Process internal destination links: [Destination Name](/destination/slug)
  ...
  html = html.replace(/^(?!<[a-z])(.+)$/gm, (match: string) => {
    ...
    if (trimmed.startsWith('|')) return match; // skip table rows
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedPosts.map(rp => (
                ...
                >
                  ...
                  <h3 className="font-bold text-gray-900 group-hover:text-sky-600 transition-colors mb-2 leading-snug">
                    {rp.title}
                  </h3>
