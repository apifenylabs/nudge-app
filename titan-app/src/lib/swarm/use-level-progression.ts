...
import { getGodTierStatus, getAbilitiesForLevel, getVisualTier } from "./god-tier-engine";
...
export function useLevelProgression(initialLevel = 1): [LevelProgressionState, LevelProgressionActions] {
  ...
  useEffect(() => {
    ...
    const godTier = getGodTierStatus(level, 0, 0, 0, 0);

    if (godTier.isGodTier && !alreadyShown) {
  ...
  const setLevel = useCallback((newLevel: number) => {
    ...
    if (newLevel > prev) {
      ...
      const prevGodTier = getGodTierStatus(prev, 0, 0, 0, 0);
      const newGodTier = getGodTierStatus(newLevel, 0, 0, 0, 0);

      // Level-up crossed the threshold: level was < 30, now >= 30
      if (!prevGodTier.isGodTier && newGodTier.isGodTier) {
        setGodTierUnlocked(true);
        setShowGodModal(true);
        sessionStorage.setItem(GOD_MODAL_SESSION_KEY, "true");
      }
